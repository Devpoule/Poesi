...
use App\Infrastructure\Repository\RelicRepository;
use JsonException;
...
#[AsCommand(
    name: 'app:lore:sync-relics',
    description: 'Sync Relic reference data from resources/lore/relics.initial.json (upsert by key).'
)]
final class SyncRelicsCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $path = rtrim($this->kernel->getProjectDir(), DIRECTORY_SEPARATOR) . '/resources/lore/relics.initial.json';
        ...
        $em = $this->relicRepository->getEntityManager();
        $keysFromJson = [];

        foreach ($rows as $i => $row) {
            $key = $this->requireString($row, 'key', $i);
            $label = $this->requireString($row, 'label', $i);
            $rarity = $this->requireString($row, 'rarity', $i);
            ...
            $relic = $this->relicRepository->findOneBy(['key' => $key]);
        ...
        if ($purge) {
            $existing = $this->relicRepository->findAll();
            foreach ($existing as $relic) {
                if (!in_array($relic->getKey(), $keysFromJson, true)) {
                    $output->writeln(sprintf('🗑️  purge: %s (%s)', $relic->getLabel(), $relic->getKey()));
                    if (!$dryRun) {
                        $em->remove($relic);
                    }
                }
        ...
        $em->flush();
        $output->writeln('<info>Relics synced successfully.</info>');
